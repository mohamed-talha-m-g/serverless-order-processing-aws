...
| No infrastructure as code | Console clicks are not reproducible, reviewable, or revertable | Full AWS SAM template on the [`iac-sam`](../../tree/iac-sam) branch |
...
| No CI | Broken code reaches deploy | GitHub Actions workflow running the test suite on every push and pull request |
