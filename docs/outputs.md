...
literal string `REGION` was left in the URL. A good demonstration of where that
failure surfaces: DNS, client-side, before AWS is involved at all.
...
because the API mapping was created with an empty path. Setup in
...
`CustomerIndex` and `StatusIndex` exist to avoid.
...
processing did not. All three queues show **Amazon SQS key (SSE-SQS)** —
encryption at rest is on by default and needs no configuration.
...
role's temporary credentials. No access keys exist anywhere in this project.
...
Lambda's billing granularity plus init; **37 MB used of 128 MB** shows plenty of
headroom — worth knowing before tuning, since on Lambda memory and CPU are the
same dial.
...
**Root cause and fix.** `An error occurred (…)` is a botocore `ClientError`; the
handler's outer `except Exception` caught it, logged it, and returned the message
as a batch item failure — correct behaviour throughout. The underlying fault was
an IAM gap on `PaymentService-role`. Adding the missing action produced the
successful run in section 3.

This is the failure mode called out in
[production-readiness.md](production-readiness.md#least-privilege-and-what-it-costs-you):
a missing IAM action is invisible at deploy time and only appears at runtime,
three retries later, in a dead letter queue.
[Step 4 of the walkthrough](console-walkthrough.md#step-4--iam-execution-roles)
has the complete policy for all three roles.
